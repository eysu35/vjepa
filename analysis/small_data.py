...
def plot_model_comparison_scatter_concise(
    ...
    if not common_categories:
        ...
        plt.xlabel(
            f"{model1_name} - {performance_metric_name} (%)", fontsize=font_size_labels
        )
        plt.ylabel(
            f"{model2_name} - {performance_metric_name} (%)", fontsize=font_size_labels
        )
    ...
    active_colors_config = (
        category_colors if category_colors is not None else DEFAULT_CATEGORY_COLORS
    )
    point_colors = []
    if isinstance(active_colors_config, dict):
        default_color_for_missing = DEFAULT_CATEGORY_COLORS[0]
        point_colors = [
            active_colors_config.get(cat, default_color_for_missing)
            for cat in common_categories
        ]
    elif (
        isinstance(active_colors_config, list) and active_colors_config
    ):  # Non-empty list
        point_colors = [
            active_colors_config[i % len(active_colors_config)]
            for i in range(len(common_categories))
        ]
    else:  # Single color string or fallback for empty list/other types
        default_color = (
            str(active_colors_config)
            if isinstance(active_colors_config, str)
            else DEFAULT_CATEGORY_COLORS[0]
        )
    ...
    padding = (
        value_range * 0.1 if value_range > 0 else 2.0
    )  # 10% padding or 2 units if no range

    plot_lim_min = max(0, data_min - padding)
    plot_lim_max = min(100, data_max + padding)
    if (
        plot_lim_max <= plot_lim_min + 1
    ):  # Ensure some visible range if data is clustered
...
def main():
    ...
    for dataset in ["inflevel", "grasp", "intphys"]:
        ...
        for exp in ["vit-l-rope-howto-0_1p_vid", "vjepa_say_base_no_rope"]:
            perf = {}
            perf_by_category_model = get_perf(
                root, exp, dataset, key_metric=key_metric, categories=categories
            )
