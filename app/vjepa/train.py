...
def main(args, resume_preempt=False):
    ...
    try:
        ...
        logger.info(
            f"Initialized evaluation DataLoader with {len(eval_loader)} batches "
        )

    except Exception as e:
        # Catch any other unexpected errors during setup
        logger.error(
            f"Failed to initialize evaluation dataset/loader: {e}", exc_info=True
        )
        ...
        for itr in range(ipe):
            ...
            if (
                ...
                if rank == 0:  # Rank 0 handles logging, comparison, and saving
                    ...
                    logger.info(
                        f"Eval Loss: {current_eval_loss:.4f} (Best: {best_eval_loss:.4f})"
                    )
                    ...
                    if is_best:
                        logger.info(
                            f"*** New best model found! Loss improved from {best_eval_loss:.4f} to {current_eval_loss:.4f} ***"
                        )
