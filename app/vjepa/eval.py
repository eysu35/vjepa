...
def evaluate(
    ...
    for i in range(len(eval_loader)):  # Iterate through the eval loader
        ...
        try:
            ...
            if not torch.isnan(loss) and not torch.isinf(loss):
                ...
                logger.warning(
                    f"[Rank {rank}] Warning: NaN or Inf loss detected during evaluation batch {i}, skipping."
                )
            ...
            logger.warning(
                f"[Rank {rank}] Eval loader iterator stopped unexpectedly at batch {i}."
            )
            break  # End of loader
        except Exception as e:
            logger.error(
                f"[Rank {rank}] Error during evaluation batch {i}: {e}", exc_info=True
            )
    ...
    if eval_loss_meter.count > 0:
        logger.info(
            f"[Rank {rank}] Finished evaluation. Avg Loss: {avg_loss:.4f} over {eval_loss_meter.count} batches. Avg Batch Time: {batch_time_meter.avg * 1000:.1f} ms. Total Time: {total_eval_time:.2f} s"
        )
    else:
        logger.warning(
            f"[Rank {rank}] Finished evaluation but processed 0 valid batches."
        )
